...
exports.getCards = async (req, res) => {
  ...
  try {
    ...
    const { startDate, endDate, league, category, ranks } = req.body.filter;
    ...
    const query = `
        ...
        DeckCardCounts AS (
            SELECT
                ...
                SUM(c.count_int) AS total_count_in_deck
            FROM cards c
            WHERE EXISTS (
                SELECT 1
                FROM FilteredDecks fd
            ...
            GROUP BY c.deck_ID_var, REPLACE(c.name_var, ' ', '')
        ),
        FilteredValidDecks AS (
            ...
            WHERE
                ${whereConditions.replace(/dcc\.count_int/g, 'dcc.total_count_in_deck')}
        ...
        PairAppearanceInDecks AS (
            SELECT
                ...
                image_var,
                name_var,
                total_count_in_deck as count_int,
                COUNT(deck_ID_var) AS appearance_count
            FROM CardCountsByDeck
            GROUP BY category_int, name_var, total_count_in_deck
            ORDER BY category_int, total_count_in_deck ASC
        )
        SELECT
            category_int,
            image_var,
    ...
    const events_count_query = `
                ...
                FROM events
                WHERE event_date_date BETWEEN ? AND ? AND event_league_int = ?;
            `;
            ...
            const decks_count_query = `
                ...
                WHERE events.event_date_date BETWEEN ? AND ? AND decks.rank_int IN (${ranks}) AND events.event_league_int = ${league};
            `
